...
const Timer = ({currentPlayer, winner, restartGame = () => console.log("No restartGame function set."), timer}) => {
    ...
    return (
        <div className={`${winner === '' ? 'h-[197px] w-[165px] -bottom-[10rem] md:-bottom-[8rem]' : 'h-[175px] w-[280px] -bottom-[4rem] md:-bottom-[4rem]'} z-[10] absolute left-0 right-0 m-auto `}>
            ...
            <div>
                ...
                <div className="absolute top-[2rem] left-0 right-0 | flex flex-col justify-center items-center">
                    <p className={`${currentPlayer === 'x' ? 'text-white' : 'text-black'}`}>PLAYER {currentPlayer === 'x' ? '1' : '2'}'S TURN</p>
                    <p className={`${currentPlayer === 'x' ? 'text-white' : 'text-black'} text-[56px]`}>{timer}s</p>
                </div>
            </div>
            :
